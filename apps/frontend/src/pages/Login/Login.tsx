...
export default function Login({ onLoginSuccess }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // INTEGRASI BACKEND AWS LAMBDA MANUAL LOGIN
    ...
    <div className="w-full max-w-110 bg-white p-8 md:border md:border-gray-200 rounded-sm flex flex-col items-center">
      ...
      <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full py-3.5 bg-[#4796ec] hover:bg-blue-600 text-white font-semibold rounded-full text-sm transition duration-200 mt-2 disabled:opacity-70 flex justify-center items-center"
        >
      ...
      <div className="w-full flex flex-col gap-3 mt-auto">
        ...
        <button
          type="button"
          onClick={() => {
            // Memicu alur Google OAuth ke Elysia Backend kamu
